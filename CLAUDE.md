...
`data-step` は `ingredients` / `options` / `result` の3つ。HTML の
`.recipe-step` を増減させたら `RECIPE_STEPS` も直す（`app-wiring.test.js`
が並びを検査する）。提案の読み込み表示は**結果画面へ移してから**出す。
入力画面に出すと折り返しの下に隠れ、押しても何も起きないように見える。
