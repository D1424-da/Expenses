...
**2026-09 上旬に検証すること。** 新方針で書いた7本がインデックスされるかで、
原因が「記事の型」なのか「サイト全体の評価」なのかが分かれる。されなかった
場合はレシピ系46本の扱いを検討することになるが、**記事を減らす判断は
運営者に確認してから**行う。
...
keep-alive は「間隔を延ばす」ではなく「**時間帯を絞る**」で対処した。
Render は15分アイドルでスリープするため、15分を超える間隔にすると
毎回スリープ済みを起こすだけになり、目的を果たさない。
