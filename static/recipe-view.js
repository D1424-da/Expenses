...
// 家族構成は Firestore（users/{uid}/settings/family）に保存し、端末をまたいでも保持する。
// 読み込み前は localStorage のキャッシュを暫定表示し、体感速度を落とさない。
function _familyRef() {
  const user = _getUser?.();
  if (!_db || !user) return null;
  return doc(_db, "users", user.uid, "settings", "family");
}
function _loadFamilyCache() {
  try { return JSON.parse(localStorage.getItem("recipe_family") || "{}"); } catch { return {}; }
}
async function _loadFamilyRemote() {
  const ref = _familyRef();
  if (!ref) return _loadFamilyCache();
  try {
    const snap = await getDoc(ref);
    if (snap.exists()) {
      const data = snap.data();
      localStorage.setItem("recipe_family", JSON.stringify(data));
      return data;
    }
  } catch (err) {
    logErr("家族構成の読み込みエラー:", err.message);
  }
  return _loadFamilyCache();
}
function _saveFamily() {
  ...
  const ref = _familyRef();
  if (ref) setDoc(ref, obj).catch((err) => logErr("家族構成の保存エラー:", err.message));
  return obj;
}
async function _restoreFamily() {
  const saved = await _loadFamilyRemote();
...
export function initRecipe({ getToken, fetchAllExpenses, getBudget, db, getUser }) {
  _getToken = getToken;
  ...
  $("recipe-suggest-btn").onclick = _onNext;
  ...
  $("recipe-select-day").addEventListener("change", (e) => {
    ...
    if (_selectResult) _renderSelectPicker();
  });
  // 時短タブ
...
export function openRecipeModal({ selectedDay, expenses, initialPeriod = "day" }) {
  ...
  if (selectedDay) {
    ...
    $("recipe-plan-end").value = dayKey(endDefault);
  } else {
    $("recipe-plan-end").value = "";
  }
...
// 週間献立の開始日〜終了日が最大7日間に収まっているか検証し、エラー時はUIに表示する。
// ---- ウィザードの段階 --------------------------------------------------------

// いま選ばれている食材の数。判定に使うので、表示と同じ集合から数える。
function _chipCount() {
  const el = _budgetMode
    ? ($("recipe-budget-chips") || $("recipe-ingredients"))
    : $("recipe-ingredients");
  return el ? el.querySelectorAll(".recipe-chip").length : 0;
}
...
function _stepState() {
  return {
    ...
    planRangeError: (_activeType === "weekly" && !$("recipe-plan-range-error").hidden)
      ? $("recipe-plan-range-error").textContent
      : "",
    busy: _busy,
  };
}

// 表示を現在の段階に合わせる。**進めない理由は必ず文言で出す** —
// ボタンを黙って disabled にすると何が足りないのか分からない。
function _renderStep() {
  const meta = stepMeta(_step);
  ...
  next.hidden = meta.nextLabel === null;
  if (meta.nextLabel) next.textContent = meta.nextLabel;

  const { ok, reason } = canAdvance(_step, _stepState());
  next.disabled = !ok;
  const hint = $("recipe-step-hint");
...
function _goStep(step) {
  _step = step;
  _renderStep();
  // 段階が変わったら先頭から読ませる。前の画面の途中位置が残ると、
  // 「何も起きていない」ように見える。
  const box = document.querySelector("#recipe-modal .modal-box");
  if (box) box.scrollTop = 0;
}

// 提案を始める前に止まった理由。結果画面の status は隠れているため、
// ナビ直上のヒント欄に出す。
function _showStepError(text) {
  const h = $("recipe-step-hint");
  h.textContent = text;
  h.hidden = false;
}
...
function _onNext() {
  const { ok, reason } = canAdvance(_step, _stepState());
  if (!ok) {
    if (reason) { const h = $("recipe-step-hint"); h.textContent = reason; h.hidden = false; }
    return;
  }
  if (_step === "options") { _suggest(); return; }
  _goStep(nextStep(_step));
}

function _validatePlanRange() {
  const start = $("recipe-plan-start").value;
  const end   = $("recipe-plan-end").value;
  const errEl = $("recipe-plan-range-error");
  if (!start || !end) { errEl.hidden = true; return true; }
  if (end < start) {
  ...
  if (days > 7) {
    errEl.textContent = `献立を作成できる期間は最大7日間です（現在${days}日間）。`;
    errEl.hidden = false;
    return false;
  }
  errEl.hidden = true;
...
async function _suggest() {
  if (!OCR_API_BASE) {
    _showStepError("バックエンドが設定されていません（firebase-config.js の OCR_API_BASE を確認してください）。");
    return;
  }
  if (_activeType === "weekly" && !_validatePlanRange()) {
  ...
  _busy = true;
  btn.disabled = true;
  // 待ち時間はここで初めて出る。結果画面へ移してから読み込み表示を出さないと、
  // 入力画面の下に隠れて「押しても何も起きない」ように見える。
  _goStep("result");
  ...
  try {
    ...
    _busy = false;
    btn.disabled = false;
    // 結果画面では「次へ」を隠し、戻るを出す。失敗時もここへ戻ってこないと
    // 再試行できない。
    _renderStep();
  }
}
...
function _renderBudgetChips() {
  ...
  _renderStep();   // チップを外して0件になったら進めない
}
